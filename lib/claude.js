...
// Pass image URLs via `images` to let Claude see them (reference selection).
export async function askJSON({ system, user, images = [], maxTokens = 8000, temperature = 0.4 }) {
  const content = images.length
    ? [...images.map((url) => ({ type: 'image', source: { type: 'url', url } })), { type: 'text', text: user }]
    : user;
