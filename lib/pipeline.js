...
import { createProject, uploadReference, startGenerate, checkJob } from './lovart.js';
  ...
  try {
    switch (step) {
      ...
      case 'verify_variants': {
        await enterStep(sb, run, step);
        const chosen = await pickVariantSmart(state.scrape, source.notes, brand);
      ...
      case 'images_start': {
        ...
        // Reference selection: Claude looks at the source photos and keeps only
        // clean catalog shots, plus writes a shape sentence to pin product fidelity.
        const candidates = (state.scrape.images || []).slice(0, 8);
        let refUrls = candidates.slice(0, 4);
        let shape = '';
        if (candidates.length) {
          try {
            const pick = await askJSON({
              system: 'You select reference photos for product image generation. Return strict JSON only.',
              user: `Product: ${state.scrape.title}\nThe images above are numbered 1..${candidates.length} in order. Return JSON {"refs":[indexes of up to 4 clean, white/plain-background catalog photos showing the full product clearly — no lifestyle scenes, no color swatches, no infographics],"shape":"one precise sentence describing the product's exact physical shape, parts, materials and true color"}`,
              images: candidates,
              temperature: 0,
            });
            const picked = (pick.refs || []).map((i) => candidates[i - 1]).filter(Boolean);
            if (picked.length) refUrls = picked;
            shape = pick.shape || '';
          } catch { /* fall back to first 4 */ }
        }
        const refIds = [];
        for (const u of refUrls) { try { refIds.push(await uploadReference(u)); } catch { /* skip */ } }
        const fidelity = `\n\nPRODUCT FIDELITY (critical): ${shape || 'match the attached reference photos exactly'}. Reproduce the exact product from the attached reference photos — same shape, parts, proportions and true color. Do not invent or restyle the product.`;
        const roles = [
          { role: 'hero', prompt: prompts.hero },
          ...prompts.gallery.map((p) => ({ role: 'gallery', prompt: p })),
          ...prompts.section.map((p, i) => ({ role: `section${i + 1}`, prompt: p })),
        ].filter((r) => r.prompt);
        const jobs = [];
        for (const r of roles) {
          const jobId = await startGenerate({ prompt: r.prompt + fidelity, referenceIds: refIds, projectId });
      ...
      case 'images_poll': {
        ...
        for (const j of jobs) {
          ...
          // "done" can briefly precede artifacts landing — give it two grace polls
          if (!r.ok && (j.emptyChecks || 0) < 2) { j.emptyChecks = (j.emptyChecks || 0) + 1; allDone = false; continue; }
          j.done = true;
          if (r.ok) {
...
export async function resumeAfterImages(sb, runId) {
  ...
  // Build with whatever the operator approved — hero leads the gallery.
  const hero = approved.filter((a) => a.role === 'hero');
  const gallery = [...hero, ...approved.filter((a) => a.role === 'gallery')];
  ...
  try {
    ...
    if (media.length) await sp.gql(`mutation($id:ID!,$media:[CreateMediaInput!]!){ productCreateMedia(productId:$id, media:$media){ mediaUserErrors{ field message } } }`, { id: productGid, media });
...
// Claude-based selection: the operator's plain-English note ("only the king",
// "the sand color") plus the real variant list decide; keyword scoring is the
// fallback when there's no note or the call fails.
async function pickVariantSmart(scrape, notes, brand) {
  const salable = (scrape.variants || []).filter((v) => v.salable);
  if (notes && salable.length > 1) {
    try {
      const res = await askJSON({
        system: `You select product variants for ${brand.name}. Return strict JSON only: {"skus": ["..."], "reason": "..."} — the skus of the variants to sell, chosen from the provided list.`,
        user: `Operator note: ${JSON.stringify(notes)}\n\nAvailable variants (salable only):\n${JSON.stringify(salable.map((v) => ({ sku: v.sku, name: v.name, color: v.color, stock: v.stock })), null, 2)}\n\nPick the variant(s) the note asks for. If the note doesn't mention variants, pick the single best fit for the brand palette: ${(brand.palette_keywords || []).join(', ')}.`,
        temperature: 0,
      });
      const picked = salable.filter((v) => (res.skus || []).includes(v.sku));
      if (picked.length) return picked;
    } catch { /* fall through to keyword scoring */ }
  }
  return pickVariant(scrape, notes, brand);
}
